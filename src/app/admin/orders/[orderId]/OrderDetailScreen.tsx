...
export default function AdminOrderDetail() {
  ...
  const isPickup = address.method === 'pickup'
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-4">
          <section className="rounded-2xl border border-line bg-white p-4 text-body">
            ...
            <span className="mt-1 inline-flex rounded-full bg-brand-tint px-2 py-0.5 text-label font-semibold capitalize text-brand">
              {address.method === 'pickup' ? 'Pickup' : 'Delivery'}
            </span>
            ...
            {order.payment_status !== 'verified' ? (
              ...
              <div className="flex flex-col gap-2">
                ...
                {order.status === 'out_for_delivery' && order.receipt_confirmed && (
                  <p className="flex items-center gap-2 rounded-lg bg-success/10 px-3 py-2 text-body text-success">
                    <CheckCircle2 size={16} />
                    {isPickup ? 'Customer confirmed pickup' : 'Customer confirmed receipt'}
                  </p>
                )}
                <ActionButton
